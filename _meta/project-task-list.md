...
- [ ] **CORE-TASK-013: Fix remaining selection capture failures in Electron editors (Codex, VS Code)** - 🟡 **IN PROGRESS** (Est: 2-4h)
  ...
  - [ ] Keep panel visible when app deactivates during capture (avoid "blink then close"): set `CorrectionPanel.hidesOnDeactivate = false` (Est: 15-30m)
  - [ ] Improve AX menu-copy fallback to reliably trigger Copy (avoid pressing wrong 'c' menu item): score candidates by cmdChar+modifiers+enabled, with title as tie-breaker (Est: 30-60m)
  - [ ] Improve synthetic Cmd+C for Electron: post only `c` key with `.maskCommand`, and try `.cghidEventTap` as an extra fallback tap (Est: 15-30m)
